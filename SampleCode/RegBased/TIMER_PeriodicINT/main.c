 ...
 * @return      None
 *
 * @details     The Timer3 default IRQ, declared in startup_NUC123.s.
 */
...
void SYS_Init(void)
{
    ...
    CLK->CLKSEL1 = CLK_CLKSEL1_UART_S_PLL |
                   CLK_CLKSEL1_TMR0_S_HXT | CLK_CLKSEL1_TMR1_S_HCLK | CLK_CLKSEL1_TMR2_S_HIRC | CLK_CLKSEL1_TMR3_S_LIRC;
...
int main(void)
{
    ...
    printf("  Timer2: Clock source HIRC;         Periodic mode; Enable interrupt; 4 interrupt ticks/sec.\n");
    printf("  Timer3: Clock source LIRC;         Periodic mode; Enable interrupt; 8 interrupt ticks/sec.\n");
    ...
    while(u32InitCount < 20)
    {
        if(g_au32TMRINTCount[0] != u32InitCount)
        {
            ...
            if((g_au32TMRINTCount[1] > (g_au32TMRINTCount[0] * 2 + 1)) || (g_au32TMRINTCount[1] < (g_au32TMRINTCount[0] * 2 - 1)) ||
                    ...
                    (g_au32TMRINTCount[3] > (g_au32TMRINTCount[0] * 8 + u32InitCount)) || (g_au32TMRINTCount[3] < (g_au32TMRINTCount[0] * 8 - u32InitCount)))
            {
                printf("*** FAIL ***\n");
                goto lexit;
            }
