 ...
 * @file     sdcard.c
 * @version  V1.00
 * @brief    NUC123 series SD Card source file
 *
 * @note
...
static uint32_t SingleWrite(uint32_t u32Data)
{
    ...
    SPI_WRITE_TX0(g_pSPI, u32Data);
    SPI_TRIGGER(g_pSPI);
    while(SPI_IS_BUSY(g_pSPI));
    return SPI_READ_RX0(g_pSPI);
}
    ...
    switch(current_command.trans_type)   // This conditional handles all data
    {
        ...
        case RDB:                         // Read data from the MMC;
            ...
            if(pchar)
            {
                ...
                {
                    SPI_WRITE_TX0(g_pSPI, 0xFF);
                    SPI_TRIGGER(g_pSPI);
                    while(SPI_IS_BUSY(g_pSPI));
                    *(pchar + counter) = SPI_READ_RX0(g_pSPI);
                }
            }
            else
            {
        ...
        case WR:
            ...
            for(counter = 0; counter < current_blklen; counter++)
            {
                SPI_WRITE_TX0(g_pSPI, *(pchar + counter));
                SPI_TRIGGER(g_pSPI);
...
/*@}*/ /* end of group NUC123_SDCARD_Driver */
