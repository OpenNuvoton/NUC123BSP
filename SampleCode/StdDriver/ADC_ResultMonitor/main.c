...
void AdcResultMonitorTest()
{
    ...
    ADC_Open(ADC, NULL, ADC_ADCR_ADMD_CONTINUOUS, 0x1 << 2);
    ...
    ADC_ENABLE_CMP0(ADC, 2, ADC_ADCMPR_CMPCOND_LESS_THAN, 0x200, 5);

    /* Enable ADC comparator 1. Compare condition: conversion result >= 0x200; match Count=5. */
    printf("   Set the compare condition of comparator 1: channel 2 is greater than or equal to 0x200; match count is 5.\n");
    ADC_ENABLE_CMP1(ADC, 2, ADC_ADCMPR_CMPCOND_GREATER_OR_EQUAL, 0x200, 5);
